...
# Lab II: Introduction to R Markdown, Logical Values, Subsetting, and Factor Variables

```{r, echo=FALSE}
knitr::include_graphics("image_2.jpg")
```
...
### 1. In the setup chunk above, load the tidyverse packages as well as library(readr)



### 2. Load in the resume.RData file and use head(), tail(), glimpse(), dim(), summary(), and View() to examine each variable in the dataset. How many of the resumes have white sounding names? How many have African-American sounding names.
...
### 4. We are going to see if there is a racial discrepency by taking the difference in callback rates between racial groups. Calculate the callback rate for white sounding name applicants and African-American sounding name applicants. Use Latex commands to write the formula for this calculation and display the result in text. Write the formula between $'s like $y = mx + b$ to use Latex commands.
...
### 5. Now, create a new object that stores the difference in callback rates named race_diff.
...
### 7. What is the difference in call back rates for each race/gender group?
