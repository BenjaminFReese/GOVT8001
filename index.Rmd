...
# Lab IX Interactions Redux

```{r, echo=FALSE}
knitr::include_graphics("image_9.png")
...
- Now let's interact `white` with `party7`
$$Human\ Cause_i = \alpha + \beta_1 Educ_i +  \beta_2 White_i + \beta_3 Party7_i + \beta_4 (White_i \times Party7_i) + \epsilon_i$$
- Interpret the following coefficients
...
- [Vignette](https://cran.r-project.org/web/packages/margins/vignettes/Introduction.html)
...
```{r, echo=FALSE}
cplot(model3, "educ", "party7", what = "effect", draw = FALSE)
...
- Let's consider a model with a squared term
$$Human\ Cause_i = \alpha + \beta_1 Age_i +  \beta_2 Age_i^2 + \epsilon_i$$
